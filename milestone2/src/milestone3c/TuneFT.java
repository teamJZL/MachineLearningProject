...
public class TuneFT {
    ...
    public static void main(String[] args) throws Exception {
        BufferedReader reader = new BufferedReader(new FileReader("data/hepatitis_train.arff"));
        ArffReader arff = new ArffReader(reader);
        Instances isTrainingSet = arff.getData();
        isTrainingSet.setClassIndex(isTrainingSet.numAttributes() - 1);

        BufferedReader reader2 = new BufferedReader(new FileReader("data/hepatitis_test.arff"));
        ...
        ps1.addCVParameter("M 11 11 1");
        ps1.addCVParameter("F 0 0 1");

        // build and output best options
        ps1.buildClassifier(isTrainingSet);
        System.out.println(Utils.joinOptions(ps1.getBestClassifierOptions()));
        
        ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("models_milestone3c/hepatitis1.model"));
        oos.writeObject(ps1);
        oos.flush();
        oos.close();
